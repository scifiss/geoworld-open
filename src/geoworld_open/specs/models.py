"""Explicit GeoSpec V2 models for the public structural workflow."""
...
class FoldStructureSpec(StrictModel):
    kind: Literal["fold"]
    id: str = Field(min_length=1, pattern=r"^[a-z][a-z0-9_]*$")
...
class FaultStructureSpec(StrictModel):
    kind: Literal["fault"]
    id: str = Field(min_length=1, pattern=r"^[a-z][a-z0-9_]*$")
    x_position_m: float
...
StructureSpec = Annotated[
    FoldStructureSpec | FaultStructureSpec,
...
class GeoSpecV2(StrictModel):
    schema_version: Literal["2.0"]
    metadata: MetadataSpec
    seed: int = Field(ge=0, le=2**63 - 1)
    grid: GridSpecV2
    facies: list[FaciesSpec] = Field(min_length=1)
    layers: list[LayerSpecV2] = Field(min_length=1)
    ...
    def validate_scientific_structure(self) -> "GeoSpecV2":
        ...
        known_facies = set(facies_ids)
        missing_facies = sorted({layer.facies_id for layer in self.layers} - known_facies)
        if missing_facies:
        ...
        for structure in self.structures:
            if isinstance(structure, FaultStructureSpec):
...
def load_geospec_v2(path: str | Path) -> GeoSpecV2:
    """Load and strictly validate a native GeoSpec V2 YAML document."""
